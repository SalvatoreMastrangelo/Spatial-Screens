...
void Telemetry::send_app(float fps, bool sixdof, bool anchored, float distance,
                         ...
                         bool stereo, int screens, int window_sources) {
    if (!enabled_) return;
    long t = now_ms();
    ...
    char buf[512];
    snprintf(buf, sizeof(buf),
             R"({"type":"app","fps":%.1f,"sixdof":%s,"anchored":%s,"distance":%.2f,"size":%.2f,"backend":"%s","direct":%s,"rss":%d,"stereo":%s,"screens":%d,"window_sources":%d})",
             fps, sixdof ? "true" : "false", anchored ? "true" : "false",
             distance, size_in, backend, direct ? "true" : "false", rss_mb,
             stereo ? "true" : "false", screens, window_sources);
    ws_.broadcast(buf);
}
